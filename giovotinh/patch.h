...
NSString *StaticInlineHookPatch(char *machoPath, uint64_t vaddr, char *patch)
{
  ...
  if (newPath)
  {
    ...
    if (!macho)
      return [NSString
          stringWithFormat:
              @"?????(can't find file):\n Documents/static-inline-hook/%s",
              machoPath];
  }
  else
  {
       ...
       i++)
  {
    if (hookBlock[i].hook_vaddr == funcRVA)
    {
      ...
      if (newPath)
        return @"??????, "
               @"??APP?Documents/"
               @"static-inline-hook???????????ipa??.app?????????!\nThe offset "
               @"is already patched! Please replace the patched file in the "
               @"APP's Documents/static-inline-hook directory to the .app "
  ...
  NSString *savePath =
      [NSString stringWithFormat:@"%@/Documents/static-inline-hook/%s",
  ...
  return @"??????, ????????APP?Documents/static-inline-hook???, "
         @"?????????????ipa??.app?????????!\nThe offset has not been patched, "
         @"the patched file will be generated in the "
         @"Documents/static-inline-hook directory of the APP, please replace "
